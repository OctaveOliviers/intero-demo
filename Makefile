...
# Pre-build the NPDA mapping.json under seed/ via the LLM mapping phase.
# Run once after spinning up the LLM (LLM_API_BASE in .env); the result is
# committed into seed/ and from then on `make seed` copies it into var/
...
# Start the API server (hot reload on :8000).
dev:
	python3 -m server
...
# a Tier-1 fill-rate of the compiled executable against the seeded databases.
# Needs `make db seed` + a reachable model for the index/mapping stages
# (models.json). `eval-tier1` is the offline leg (no LLM).
eval:
	python3 -m scripts.eval_pipeline --stage all
eval-tier1:
	python3 -m scripts.eval_pipeline --stage tier1-golden
