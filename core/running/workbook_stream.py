"""Build a run's workbook purely from its state-DB cells.

state.db is the single source of truth: the spine writes every cell value there
and streams it to the live grid, but nothing is ever written back to disk. The
cells are self-describing — each carries its ``SHEET!A1`` ref (sheet + position)
and its value — so the sheets, columns and rows can be reconstructed from the
refs alone. There is no on-disk ``result.xlsx`` and no separate layout to drift
against: a rebuilt workbook can only ever match what the run actually wrote.
...
def build_cell_workbook(cells: Iterable, field_names: dict | None = None) -> Workbook:
    ...
    cohort grid renders before any tier fills a value.
    """
