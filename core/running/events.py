...
def _needs_review_cells(cells: list[Cell]) -> list[Cell]:
    ...
    return [
        c
        for c in cells
        if c.state == "filled" and c.review_state == "not_reviewed"
    ]
    ...
    low_confidence = [
        {
            ...
            "rationale": c.explanation or c.reason_detail or c.hypothesis
...
def _review_counts(cells: list[Cell]) -> tuple[dict[str, int], dict[str, int], dict[str, int]]:
