...
each, used by every writer so the hand-rolled dicts (and their required fields)
...
def make_attempt(
    ...
    *,
    ...
    """One ``attempts[]`` entry. ``by`` (``prepopulate`` | ``agent``) +
    ``database`` always present (``database`` is ``"(none)"`` when no query
    ran); the rest only when set."""
    attempt: dict = {"by": by, "database": database}
    for key, val in (
        ("sql", sql),
        ("value", value),
