...
+ template/ + database/ — exactly the shape the orchestrator will lay down.
...
class AgentToolsTest(unittest.TestCase):
    def setUp(self):
        ...
        (self.worktree / "template" / "spec.json").write_text(
            json.dumps(_audit()), "utf-8"
        )
        (self.worktree / "template" / "cells.sqlite").symlink_to(
            self.state_db.resolve()
        )
        (self.worktree / "databases").mkdir()
        (self.worktree / "databases" / "cord-ph.sqlite").symlink_to(
            self.db_path.resolve()
        )
        (self.worktree / "databases" / "cord-ph.model.json").write_text(
    ...
    def test_hospital_query_touching_no_cohort_table_is_rejected(self):
        res = self._tool(
            "sql_execute.py", {"database": "cord-ph", "sql": "SELECT 1 AS x"}
        )
    ...
    def test_chat_query_rejects_table_valued_schema_pragma(self):
        (self.worktree / "context.json").write_text(
            json.dumps(
                build_context(
                    mode="chat",
        ...
        res = self._tool(
            ...
            {
                "database": "cord-ph",
                "sql": "SELECT * FROM pragma_table_info('cord_ph_birth_records')",
            },
        )
        self.assertFalse(res["ok"])
        self.assertIn("catalog", res["error"].lower())

    def test_chat_query_rejects_bare_schema_pragma_table(self):
        (self.worktree / "context.json").write_text(
            json.dumps(
                build_context(
                    mode="chat",
        ...
        res = self._tool(
            "sql_execute.py",
            {"database": "cord-ph", "sql": "SELECT * FROM pragma_database_list"},
        )
        self.assertFalse(res["ok"])
        self.assertIn("catalog", res["error"].lower())
    ...
    def test_cells_in_code_write_with_source_fills_and_stamps_attempt(self):
        ...
        self.assertEqual(cell.attempts[-1]["by"], "agent")
        ...
        self.assertEqual(
            cell.sources[0]["table_column"], "cord_ph_birth_records.delivery"
        )
    ...
    def test_cells_cross_run_write_denial_is_sanitized(self):
        ...
        self.assertFalse(res["ok"])
        self.assertEqual(
            res["error"], "Permission denied for requested database operation."
        )
    ...
    def test_single_interpret_fill_allowed_and_note_scoped_to_row(self):
        ...
        note = (
            '[{"database":"cord-ph","query":"SELECT patient_code, note_id, note_text '
    ...
    def test_lookup_database_read_redirects_to_navigate(self):
        # The {database}/{database,table} selectors left lookup for the navigate
        # tools; a database read now redirects rather than serving a digest.
...
def _npda_clinical_model() -> dict:
    return {
        ...
        "foreign_keys": [
            {
                "column": "measurements.visit_ref",
                "target": "clinic_visits.visit_id",
                "cardinality": "to-many",
                "declared": False,
                "evidence": "5/5",
            },
        ...
        "tables": [
            ...
            },
            {
                ...
                "grain": "one row per patient screening result",
                "columns": [
                    {"name": "screening_id"},
                    {"name": "patient_ref"},
                    {"name": "programme"},
        ...
        "foreign_keys": [
            {
                ...
                "target": "registrations.patient_id",
                "cardinality": "to-one",
                "declared": False,
                "evidence": "10/10",
            },
            {
                "column": "registration_notes.patient_id",
...
class CrossDatabaseTest(unittest.TestCase):
            ...
            json.dumps(
                build_context(
                    ...
                    databases={
                        ...
                        "npda-demographics": {
                            "cohort_tables": [],
                            "identity_links": _npda_demographics_model()[
                                "identity_links"
                            ],
    ...
    def test_foreign_db_bridge_table_succeeds_and_is_cohort_scoped(self):
        ...
        self.assertTrue(res["ok"], res)
        self.assertEqual(sorted(r["nhs_number"] for r in res["rows"]), ["NHS1", "NHS2"])
    ...
    def test_foreign_db_to_one_hop_from_bridge_table_succeeds(self):
        ...
        self.assertTrue(res["ok"], res)
        self.assertEqual(
            sorted(r["registration_id"] for r in res["rows"]), ["R1", "R2"]
        )  # R_OUT excluded
    ...
    def test_cross_db_join_anchor_to_foreign(self):
        ...
        self.assertEqual(
            {r["family_name"] for r in res["rows"]}, {"Patel", "Whitfield"}
        )
    ...
    def test_anchor_db_query_still_cohort_scoped(self):
        ...
        res = self._tool(
            {"database": "npda-clinical", "sql": "SELECT visit_id FROM clinic_visits"}
        )
        self.assertTrue(res["ok"], res)
        self.assertEqual(
            sorted(r["visit_id"] for r in res["rows"]), ["V1", "V2", "V3"]
        )  # V_OUT excluded
    ...
    def test_to_many_child_of_anchor_is_reachable_via_exists(self):
        ...
        self.assertTrue(res["ok"], res)
        self.assertEqual(
            sorted(r["measurement_id"] for r in res["rows"]), ["M1", "M2", "M3", "M4"]
        )  # M_OUT excluded
    ...
    def test_parenthesized_table_group_is_rejected_not_leaked(self):
        ...
        for sql in (
            ...
            self.assertFalse(
                res["ok"], f"expected rejection, got rows for: {sql}\n{res}"
            )
    ...
    def test_unreachable_table_fails_safe(self):
        ...
        res = self._tool(
            {"database": "npda-demographics", "sql": "SELECT * FROM sqlite_master"}
        )
        self.assertFalse(res["ok"])
...
class DirectedPathAmbiguityTest(unittest.TestCase):
    ...
    def test_ambiguous_directed_paths_fail_closed_and_order_invariant(self):
        ...
        fk1 = {
            "column": "mirrored_obs.left_id",
            "target": "visits.right_id",
            "cardinality": "to-one",
            "declared": False,
            "evidence": "n/a",
        }
        fk2 = {
            "column": "mirrored_obs.right_id",
            "target": "visits.left_id",
            "cardinality": "to-one",
            "declared": False,
            "evidence": "n/a",
        }
...
class ScopeSourceToCellTest(unittest.TestCase):
        ...
        for q in ("UPDATE t SET x=1", "this is not sql ("):
            self.assertEqual(
                scope_source_to_cell(
                    q, anchor="patient_code", member="P001", cohort_tables=self.CT
                ),
                q,
            )
    ...
    def test_nested_query_returned_verbatim(self):
        original = (
            "SELECT patient_code FROM cord_ph_birth_records "
            "WHERE delivery IN (SELECT delivery FROM cord_ph_birth_records)"
        )
        out = scope_source_to_cell(
            original, anchor="patient_code", member="P001", cohort_tables=self.CT
...
class ChatModeTest(unittest.TestCase):
    """Chat-mode sql_execute (Q40): a permission-bounded read mode with NO cohort
    injection. A plain SELECT reads the whole registered read-only DB (rows the
    run-mode cohort filter would exclude come back); a write is rejected; an unknown
    database fails closed; ``database="cells"`` is rejected (a chat has no worksheet).

    Contrast with the run mode above: there the cohort is INJECTED and a row outside
    it (``P_OUT``) is filtered out without the agent asking. In chat there is no
    cohort, so the agent reads the whole table — bounded only by the registered
    read-only databases (the per-message ceiling; Q37 v1)."""
    ...
    def setUp(self):
        ...
        self.db_path = self.worktree / "hospital.sqlite"
        _hospital_db(self.db_path)
        # A chat context: mode="chat", databases only — NO run_id/anchor/cohort.
        (self.worktree / "context.json").write_text(
            json.dumps(
                build_context(
                    mode="chat",
    ...
    def test_chat_context_has_no_run_keys(self):
        # The chat context shape is mode + databases only — no run_id/anchor/cohort.
        ctx = json.loads((self.worktree / "context.json").read_text())
        self.assertEqual(ctx["mode"], "chat")
        self.assertNotIn("run_id", ctx)
        self.assertNotIn("anchor", ctx)
        self.assertNotIn("cohort", ctx)
        self.assertIn("cord-ph", ctx["databases"])

    def test_plain_select_reads_whole_db_without_cohort_scoping(self):
        # No cohort is injected: P_OUT (excluded by the run-mode cohort filter)
        # is returned. The agent wrote no scope, and the read is the whole table.
        res = self._tool(
            {
                "database": "cord-ph",
                "sql": "SELECT patient_code FROM cord_ph_birth_records "
        ...
        self.assertTrue(res["ok"], res)
        self.assertEqual(
            [r["patient_code"] for r in res["rows"]], ["P001", "P002", "P_OUT"]
        )

    def test_aggregate_query_is_allowed(self):
        # Aggregates / GROUP BY are legitimate in chat (a "how many" answer);
        # reject_nested_queries is NOT applied, so this runs.
        res = self._tool(
            {
                "database": "cord-ph",
        ...
        self.assertTrue(res["ok"], res)
        self.assertEqual(res["rows"][0]["n"], 3)

    def test_subquery_is_allowed_in_chat(self):
        # A read-only subquery is legitimate in chat (Issue 5 needs them);
        # reject_nested_queries does NOT apply in chat mode.
        res = self._tool(
            {
                "database": "cord-ph",
                "sql": "SELECT patient_code FROM cord_ph_birth_records "
                "WHERE cord_arterial_ph < (SELECT AVG(cord_arterial_ph) "
                "FROM cord_ph_birth_records)",
            }
        )
        self.assertTrue(res["ok"], res)

    def test_sqlite_catalog_reads_are_rejected_in_chat(self):
        for sql in (
            "SELECT name FROM sqlite_master",
            "SELECT name FROM main.sqlite_schema",
            "SELECT name FROM sqlite_sequence",
        ):
            res = self._tool({"database": "cord-ph", "sql": sql})
            self.assertFalse(res["ok"], sql)
            self.assertIn("navigate", res["error"].lower())

    def test_write_is_rejected_read_only(self):
        res = self._tool(
            {
                "database": "cord-ph",
                "sql": "UPDATE cord_ph_birth_records SET delivery='9'",
    ...
    def test_unknown_database_fails_closed(self):
        res = self._tool({"database": "not-a-db", "sql": "SELECT 1 AS x"})
        self.assertFalse(res["ok"])
        self.assertIn("not-a-db", res["error"])

    def test_cells_database_is_rejected_in_chat(self):
        res = self._tool({"database": "cells", "sql": "SELECT * FROM cells"})
        self.assertFalse(res["ok"])
        self.assertIn("worksheet", res["error"].lower())

    def test_multiple_statements_rejected(self):
        res = self._tool(
            {
                "database": "cord-ph",
                "sql": "SELECT patient_code FROM cord_ph_birth_records; SELECT 1",
            }
        )
        self.assertFalse(res["ok"])
        self.assertIn("single", res["error"].lower())

    def test_chat_query_is_written_to_local_query_log(self):
        res = self._tool(
            {
                "database": "cord-ph",
                "sql": "SELECT COUNT(*) AS n FROM cord_ph_birth_records",
        ...
        self.assertTrue(res["ok"], res)
        log_path = self.worktree / "query_log.jsonl"
...
class CitationGuardTest(unittest.TestCase):
    """A fill whose source reads a FREE-TEXT column must quote a verbatim substring
    of the cited record (sql_execute._reject_uncited_note_fill): no citation, or a
    fabricated one, is rejected and rolls the cell back to pending; a real substring
    is accepted. A structured-column fill needs no citation."""

    def setUp(self):
        self._dir = tempfile.TemporaryDirectory()
        self.worktree = Path(self._dir.name)
        self.db_path = self.worktree / "clinic.sqlite"
        conn = sqlite3.connect(self.db_path)
        conn.executescript(
            "CREATE TABLE notes (note_id TEXT PRIMARY KEY, patient TEXT, body TEXT);"
            "INSERT INTO notes VALUES ('N1','P001','seen in clinic; hybrid closed loop pump');"
            "CREATE TABLE labs (patient TEXT PRIMARY KEY, hba1c TEXT);"
            "INSERT INTO labs VALUES ('P001','51');"
        )
        conn.commit()
        conn.close()
        self.state_db = self.worktree / "state.db"
        audit = {
            "audit": "clinic",
            "fields": [
                {"id": "regime", "name": "Insulin regime", "type": "text"},
                {"id": "hba1c", "name": "HbA1c", "type": "number"},
            ],
        }
        store = Store(self.state_db)
        store.create_run(Run(id="r1", audit_id="clinic", status="in_progress"))
        store.materialize_field_codes("r1", audit)
        store.insert_pending_cells(
            [
                Cell(
                    run_id="r1",
        ...
        (self.worktree / "context.json").write_text(
            json.dumps(
                build_context(
                    ...
                    anchor="patient",
                    cohort=["P001"],
                    databases={"clinic": {"cohort_tables": ["notes", "labs"]}},
                )
            ),
            encoding="utf-8",
        )
        (self.worktree / "template").mkdir()
        (self.worktree / "template" / "spec.json").write_text(
            json.dumps(audit), "utf-8"
        )
        (self.worktree / "template" / "cells.sqlite").symlink_to(
            self.state_db.resolve()
        )
        (self.worktree / "databases").mkdir()
        (self.worktree / "databases" / "clinic.sqlite").symlink_to(
            self.db_path.resolve()
        )
        (self.worktree / "databases" / "clinic.model.json").write_text(
            json.dumps(
                {
                    "database": "clinic",
                    "tables": [
                        {
                            ...
                            "columns": [
                                {"name": "note_id"},
                                {"name": "patient"},
                                {"name": "body", "reason": "free-text"},
                            ],
                        },
                        {
                            "name": "labs",
                            "columns": [{"name": "patient"}, {"name": "hba1c"}],
                        },
                    ],
                }
            ),
    ...
    def _state(self, ref: str) -> str:
        s = Store(self.state_db)
        try:
            return {c.ref: c for c in s.get_cells("r1")}[ref].state
        finally:
            s.close()

    def _fill_regime(self, citations):
        src = {
            "database": "clinic",
            "query": "SELECT patient, note_id, body FROM notes WHERE note_id='N1'",
            "table_column": "notes.body",
            "row_id": "N1",
            "row_key": "note_id",
        }
        if citations is not None:
            src["citations"] = citations
        sources = json.dumps([src]).replace("'", "''")  # SQL-escape inner quotes
        return self._tool(
            {
                "database": "cells",
                "sql": "UPDATE cells SET value='5', state='filled', confidence='high', "
                f"resolved_by='agent', explanation='from note', sources='{sources}' "
                "WHERE ref='ALL!U2'",
            }
        )

    def test_free_text_fill_without_citation_rejected(self):
        res = self._fill_regime(None)
        self.assertFalse(res["ok"], res)
        self.assertIn("citations", res["error"])
        self.assertEqual(self._state("ALL!U2"), "pending")  # rolled back

    def test_fabricated_citation_rejected(self):
        res = self._fill_regime(["patient is a smoker"])  # not in the note body
        self.assertFalse(res["ok"], res)
        self.assertIn("verbatim", res["error"])
        self.assertEqual(self._state("ALL!U2"), "pending")  # rolled back

    def test_real_substring_citation_accepted(self):
        res = self._fill_regime(["hybrid closed loop"])  # exact substring of the note
        self.assertTrue(res["ok"], res)
        self.assertEqual(self._state("ALL!U2"), "filled")

    def test_structured_fill_needs_no_citation(self):
        src = json.dumps(
            [
                {
                    "database": "clinic",
                    "query": "SELECT patient, hba1c FROM labs",
                    "table_column": "labs.hba1c",
                }
            ]
        ).replace("'", "''")
        res = self._tool(
            {
                "database": "cells",
                "sql": "UPDATE cells SET value='51', state='filled', confidence='high', "
                f"resolved_by='agent', explanation='lab', sources='{src}' WHERE ref='ALL!S2'",
            }
        )
        self.assertTrue(res["ok"], res)
        self.assertEqual(self._state("ALL!S2"), "filled")
