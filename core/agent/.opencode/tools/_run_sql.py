...
def build_context(
    ...
    * ``databases`` — per slug, the navigation metadata the tool needs to bound a
      query to the cohort WITHOUT the agent asking:
        - ``cohort_tables``   — tables that physically carry the anchor column.
        - ``identity_links``  — cross-database bridges (``{column, target}``):
          this DB's bridge column whose values equal a sibling DB's key, so the
          cohort's anchor identities can be translated into that sibling's key
          set (mirrors the Tier-1 identity bridge).
        - ``foreign_keys``    — within-database join hops (``{column, target,
          cardinality}``). The scope walks a to-one hop either way and DESCENDS a
          to-many hop (parent → child); the EXISTS leaf-correlation keeps the
          to-many fan-out inside the subquery (see the Tier-1/Tier-3 note below).
      Drawn from each database's canonical ``model.json``; the tool consumes the
      same map Tier 1 does, never a hand-authored copy.
...
def reject_multiple_statements(sql: str) -> None:
    ...
    try:
        ...
        statements = [s for s in sqlglot.parse(
            _normalize_backslash_single_quote(sql), read="sqlite") if s is not None]
    except sqlglot.errors.SqlglotError as exc:
        raise ToolError(f"SQL parse error: {exc}") from exc
    if len(statements) > 1:
        raise ToolError(
...
#
# Shares the Tier-1 executor's NAVIGATION inputs (core/running/try_direct +
...
def _key_roots_for(
    owner_db: str, anchor_db: str, databases: dict[str, dict[str, Any]], anchor: str,
...
def _scope_path(
    table: str, key_table: str, foreign_keys: list[dict[str, Any]],
    ...
    for p in paths:
        if len(p) == shortest:
            canon = tuple(
                (h["from_table"], h["to_table"], h["from_col"], h["to_col"])
                for h in p
            )
            distinct[canon] = p
...
def _cohort_subquery_sql(
    anchor_db: str, target_db: str, bridge_table: str, bridge_col: str,
    anchor: str, cohort: list[Any],
) -> str:
...
def _bound_key_sql(
    key_ref: str, target_db: str, anchor: str, cohort: list[Any],
    anchor_db: str, via: tuple[str, str] | None,
) -> str:
...
def _scope_predicate(
    alias: str, table: str, owner_db: str, target_db: str, anchor: str,
    cohort: list[Any], databases: dict[str, dict[str, Any]], anchor_db: str,
) -> exp.Expression | None:
    ...
    for key_table, key_col, via in key_roots:
        ...
        if not hops:  # None = unreachable/ambiguous path, [] handled by direct roots.
            continue
        hop_sig = tuple(
            (h["from_table"], h["from_col"], h["to_table"], h["to_col"])
            for h in hops
        )
    ...
    for mode, key_table, key_col, via, hops in strategies.values():
        if mode == "direct":
            bound = _bound_key_sql(
                f"{_qid(alias)}.{_qid(key_col)}", target_db, anchor, cohort, anchor_db, via
            )
            predicates.append(parse(f"SELECT 1 WHERE {bound}").find(exp.Where).this)
            continue
        assert hops is not None
        predicates.append(_exists_over_path(
            alias, table, owner_db, target_db, key_table, key_col, hops,
            anchor, cohort, anchor_db, via,
        ))
...
def _exists_over_path(
    outer_alias: str, table: str, owner_db: str, target_db: str, key_table: str,
    key_col: str, hops: list[dict[str, str]], anchor: str, cohort: list[Any],
    anchor_db: str, via: tuple[str, str] | None,
) -> exp.Expression:
    ...
    bound = _bound_key_sql(
        f"{_qid(h(0))}.{_qid(key_col)}", target_db, anchor, cohort, anchor_db, via,
    )
...
def inject_cohort(tree: exp.Expression, anchor: str, cohort: list[Any],
                  databases: dict[str, dict[str, Any]], target_db: str) -> exp.Expression:
    ...
    if not tables:
        hint = (f" The cohort-bearing tables in this database are: {cohort_tables}."
                if cohort_tables else "")
        raise ToolError(
    ...
    for table in tables:
        ...
        if alias.lower().startswith(_SCOPE_ALIAS_PREFIX) or name.startswith(_SCOPE_ALIAS_PREFIX):
            raise ToolError(
        ...
        pred = _scope_predicate(
            alias, name, owner_db, target_db, anchor, cohort, databases, anchor_db,
        )
        if pred is None:
            owner_cohort = sorted(databases.get(owner_db, {}).get("cohort_tables") or [])
            cohort_hint = (f" The cohort-bearing tables in {owner_db!r} are: "
                           f"{owner_cohort}." if owner_cohort else "")
            raise ToolError(
                ...
                f"bounded to the audited cohort. Use lookup_execute({{\"database\": "
                f"{owner_db!r}}}) to see the foreign_keys/identity_links you can join "
                f"through, and read from a table the map can reach.{cohort_hint}"
            )
...
def _has_eq_literal(tree: exp.Expression, column: str, value: Any) -> bool:
    ...
    for eq in tree.find_all(exp.EQ):
        ...
        if col is not None and lit is not None \
                and col.name.lower() == column.lower() and lit.this == want:
            return True
    return False


def inject_identity(tree: exp.Expression, anchor: str, member: Any,
...
def scope_source_to_cell(query: str, *, anchor: str, member: Any,
                         cohort_tables: set[str], row_key: str | None = None,
                         row_id: Any | None = None) -> str:
    ...
    try:
        ...
        return query
    if not is_select(tree) or any(node is not tree for node in tree.find_all(exp.Select)):
        return query
    before = render(tree)
