...
that executable runs a clean prepopulation pass on the seed.

Covers the A4 checklist (specs/mvp/BUILD-PLAN.md):
  ...
  * the executable runs a clean prepopulation pass: `prepopulate` fills the seed's direct cells
...
SCHEMA = json.loads(
    (REPO_ROOT / "specs/product/contracts/mapping.schema.json").read_text(
        encoding="utf-8"
    )
)
...
class SeedMappingTest(unittest.TestCase):
    def test_seed_mapping_validates_with_executable_and_multi_db_shape(self):
        ...
        for field in MAPPING["fields"]:
            for src in field["sources"]:
                self.assertRegex(
                    src, r"^\S.*->\s*\w+\.\w+$", f"unlocatable source {src!r}"
                )
    ...
    def test_cohort_block_matches_the_criteria_base(self):
        ...
        joined = {tok for tok in cohort["from"].split() if tok in needed}
    ...
    def test_kind_is_derived_multi_source_becomes_interpret(self):
        ...
        # to interpret (prepopulate can only copy one column → one cell).
        self.assertEqual(
            _field_kind({"kind": "direct", "sources": ["a -> t.c"]}), "direct"
        )
        self.assertEqual(
...
def _multi_db_mapping() -> dict:
    ...
    return {
        ...
        "criteria_bindings": [
            {
                "criterion_id": "sex",
                "source": "demographics -> patients.sex_code",
                "join_path": "visits.patient_ref -> patients.nhs_number",
            },
            {
                "criterion_id": "ward",
                "source": "clinical -> wards.name",
...
class CrossDatabaseCompileTest(unittest.TestCase):
    ...
    keeps cross-database joins out of the cohort SQL, and forces fields prepopulate
    ...
    def test_cohort_sql_never_joins_a_foreign_database_table(self):
        ...
        self.assertNotIn(
            "patients", cohort["from"], "cross-database join never baked in"
        )
    ...
    def test_foreign_query_is_keyed_by_its_own_identity_column(self):
        ...
        self.assertNotIn(
            "key_column", by_db["clinical"], "anchor db keeps the anchor key"
        )
    ...
    def test_foreign_non_key_table_field_is_forced_to_interpret(self):
        ...
        self.assertIn(
            "diabetes_type",
            interpret_fields,
            "registrations carries no demographics key — prepopulate cannot copy it",
        )
        # Its evidence is likewise not mechanically fetchable: no registrations query.
        self.assertNotIn(
            "registrations", " ".join(q["sql"] for q in interpret["queries"])
        )
...
class SeedPrepopulatePassTest(unittest.TestCase):
    """The executable runs a clean prepopulation pass on a DB built from its own cell map."""
    ...
    def setUp(self):
        ...
        exe = MAPPING["executable"]
        # Pre-insert the pending grid (populate_table's job, mirrored here so
        # prepopulate UPDATEs in place — it never INSERTs).
        self.store.insert_pending_cells(
            table_population.build_pending_table_cells("r1", exe, self.cohort)
        )
    ...
    def test_direct_cells_are_filled_with_provenance(self):
        # Every direct cell of every cohort member is filled, resolved_by prepopulated,
    ...
    def test_code_translation_applied(self):
        # `delivery` stores text in the DB; the executable's code_map translates it to a code.
...
def _build_db_from_executable(path: Path, executable: dict, cohort: list[str]) -> None:
    ...
    and insert one clean row per cohort member — so prepopulate has a clean hit everywhere.
