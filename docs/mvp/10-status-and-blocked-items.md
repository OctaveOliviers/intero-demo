...
Read [2-product-flows.md](./2-product-flows.md), [5-run-engine.md](./5-run-engine.md), and
[6-traceability-evidence.md](./6-traceability-evidence.md) first. This document specifies how
the platform surfaces **why an audit cannot be completed — down to the individual missing
cell — and the per-audit status lifecycle** that makes those gaps visible and actionable.
...
| In the MVP today | **new** (this document) | the **interpretive safety gate** ([6-traceability-evidence.md](./6-traceability-evidence.md)): interpret cells `not_reviewed → reviewed` |

**NEEDS VERIFICATION already exists** in the MVP as the interpretive safety gate (doc 6):
...
[doc 5 §Cell state model](./5-run-engine.md#cell-state-model); this extends the per-cell
metadata in [6-traceability-evidence.md](./6-traceability-evidence.md)):

- **`pending`** — created up front by the orchestrator, not yet settled by a tier (transient
...
per [open-questions.md](./open-questions.md) Q10). It routes to the review gate (doc 6).
...
| `NOT_LOCATED` | the repair agent searched the cohort and could not place the value anywhere; the cell's value is left blank and its `attempts[]` records every query tried ([5-run-engine.md](./5-run-engine.md); design rationale archived in 0ld/run-population-redesign.md) |
...
A blocked reason is **evidence-grade**: like a filled cell's citation (doc 6), it must state
...
**Not in the workbook.** The workbook is always the **clean audit template you would send** — a
...
1. **The agent's final summary message** — the primary surface. When a run finishes, the agent
   ...
   `review_summary` event; [2-product-flows.md](./2-product-flows.md),
   [doc 11 §agent_activity](./11-result-view-workbook-first.md)) stating what it completed and
   **explicitly listing which values are blocked and why / who** — e.g. *"Audit populated.
   ...
   band's **blocked / needs-review counters** (doc 11) mirror the two queues at a glance.
2. **The audit-level status** — the Blocked status + count on the dashboard
   ([Status lifecycle](#audit-status-lifecycle-kanban)), for triage. *(The dashboard surface
...
[6-traceability-evidence.md](./6-traceability-evidence.md)) — that metadata powers the message,
...
> **persisted per-cell `state`/`reason`** plus the audit's status, so it is always available on
...
If refresh marked members as inactive/departed (out of cohort), they remain visible in
run history/UI context but are **excluded from the exported workbook**.

---

## Audit status lifecycle (Kanban)

Each **audit run** carries a primary status:

- **QUEUED** — not yet started.
- **IN PROGRESS** — extraction running.
...
Rules:
- An audit is **BLOCKED whenever it has ≥1 `blocked` cell**, and the **blocked count is
  surfaced**.
...
## Dynamic re-run / auto-resolution

Because **timing is the root cause**, re-running is first-class:

- Audits are **re-runnable / refreshable**. On each re-run, cells whose source data has since
  landed **auto-resolve** (`blocked → filled`, entering the verification queue when interpret).
- **A re-run never overwrites human work.** It only fills `blocked` or still-unfilled cells; a
  ...
  replaced. *(GAP-1, 2026-06-04 rigor review.)*
- The audit **moves out of BLOCKED automatically** once the blocked count hits zero — **no
  manual edit required**.
- **Re-run** (restart) is a fresh run pinned to the same template version
  ([9-library-and-sources.md](./9-library-and-sources.md)) and recorded in the run log
  ([7-auth-and-audit-log.md](./7-auth-and-audit-log.md)).
- **Refresh** (Check for updates) is in-place on the same `run_id` with a new
  `execution_id` (see [11-refresh-detection-and-incremental-refresh.md](./11-refresh-detection-and-incremental-refresh.md)).
...
> view's top band (doc 11).

- A **Kanban board**: **columns by status**, **cards = audit runs**.
...
- **Sending is a human-initiated, gated action** — the platform **never sends silently**
  ([7-auth-and-audit-log.md](./7-auth-and-audit-log.md)). The person running the audit reviews
  and sends.
- MVP: an in-app grouped list + drafted reminder. Channel integration (email/messaging) is an
  open question.
...
Per [7-auth-and-audit-log.md](./7-auth-and-audit-log.md): a drafted reminder names
...
- The workbook is always the **clean audit template**: a cell the agent can't fill is **left
...
- Blocked items are surfaced in the **agent's final summary message** (the terminal
  ...
  and the **chase list** are next-phase surfaces — never in the workbook.
- **BLOCKED and NEEDS-VERIFICATION items are distinguishable and never conflated** (separate
  states, queues, board columns).
- Each audit run has a **primary status** (Queued / In progress / Blocked / In verification /
  Complete), visible on the dashboard Kanban and anywhere the audit is listed.
- An audit shows as **Blocked whenever it has ≥1 blocked cell**, with the count surfaced.
...
- **Reminder channel:** in-app list only for the MVP, or integration with email/messaging
...
- **Merge IN VERIFICATION and BLOCKED** into one "Needs attention" column for MVP simplicity?
