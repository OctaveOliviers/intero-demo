...
**Status: FROZEN (Wave 0 · W0.3).** This is the shared contract for the three runtime data
shapes the run touches: the **SSE event stream** the run emits, the **per-cell metadata**
object every populated cell carries, and the **executor spec** (the compiled Tier-1 plan) the
fixed executor reads. Lock these so the three lanes can build in parallel against the shape,
not each other's code:

- **Lane A** (`core/indexing`, `core/mapping`, `core/running` executor) **writes** cells and
  **produces** the executor spec.
- **Lane B** (`core/running`, `server/routes/runs.py`) **emits** the SSE events.
- **Lane D** (`app/`) **renders** the events and the cell metadata.

> **Note (post-A4).** The §3 spec below was originally written as a separate
> `populate.json` file. The MVP folds it into `mapping.json` as the "compiled executable"
> half — same shape, same fields, same validator. See
> [`storage-layout.md`](./storage-layout.md) for the canonical layout and
> [BUILD-PLAN.md A4](../BUILD-PLAN.md) for the migration. The §3 shape itself is unchanged.

Spec sources: [5-run-engine.md §Streaming](../5-run-engine.md#streaming-the-run),
[6-traceability-evidence.md §Per-cell metadata](../6-traceability-evidence.md#per-cell-metadata),
[4-indexing-and-mapping.md §Phase 3](../4-indexing-and-mapping.md#phase-3--precompute-the-executable-block).
Reason-code taxonomy and per-cell `state`:
[10-status-and-blocked-items.md](../10-status-and-blocked-items.md). Existing demo event model:
...
The run streams Server-Sent Events to the front end as a single user-facing timeline. Exactly
...
discriminator is `data.type`. No `event:` line is required for run events.
...
- `refresh_summary` is emitted **exactly once on refresh executions only** (doc 11), after
  `review_summary` and before `done`; an initial run never emits it.
...
| `review_summary` | `{ totals, blocking, verification }` | the reviewer-focused terminal summary — the **final entry in the agent-activity feed** (doc 11) |
...
- `totals` with required integer fields: `cells`, `filled`, `blocked`, `needs_verification`
  (the **derived** needs-verification count — doc 5 §Cell state model), `low_confidence`.
...
```
Refresh executions only (doc 11): emitted once, after `review_summary` and before `done`.
All fields required integers. An initial run never emits this event.

### `done`
```jsonc
{}
```
Terminal. Marks the run complete for streaming purposes (primary status is derived from
persisted cells, not this event — doc 10). Emits no second chip. On a refresh execution,
...
> JSON Schema is the machine-checkable source of truth for the cell, the triage decision, and the
> recipe. This section is its prose companion — keep the two in sync. The cell object is produced by
> every tier of the resolution ladder (direct executor, triage LLM, repair agent) and passed up the
> escalation chain; see [5-run-engine.md](../5-run-engine.md) (design rationale archived in
> [0ld/run-population-redesign.md](../0ld/run-population-redesign.md)).
>
...
```jsonc
{
  ...
  "state": "filled",                // "pending" | "filled" | "blocked" | "not_applicable"  (REQUIRED — the four stored states, doc 5 §Cell state model; needs-verification is derived, never stored)
  "confidence": "high",             // "low" | "medium" | "high"  — a WORD, never a number  (REQUIRED)
  "resolved_by": "direct",          // "direct" | "LLM" | "agent" — which ladder tier produced it  (REQUIRED)
  "hypothesis": null,               // why the direct pass failed; null on a clean direct hit (written by the first LLM to see the failure)
  "explanation": "From the EHR birth record for P-0001 — gestation in completed weeks.",

  // --- attempts: the log of ACTUAL DB queries, in order; the escalation payload (REQUIRED, >=1) ---
  // A clean direct hit is exactly one entry with no error. A new entry is appended only when a
  // query is really run (a triage retry, an agent query) — not for LLM reasoning over fetched data.
  "attempts": [
    { "tier": "direct", "sql": "SELECT PATIENT_CODE, Gestation_weeks FROM cord_ph_birth_records WHERE PATIENT_CODE = :patient", "result": "39" }
  ],
  ...
  // --- interpret-only ---
  "review_state": "not_reviewed",   // "not_reviewed" → "reviewed" (set automatically after ~2s view; doc 6)
  "corrected": false,               // false = left as-is (confirmed) · true = edited (the accuracy signal)

  // --- provenance (all cells) ---
  "prompt_version": "audit-run@2026-06-01",  // ties to the run record (doc 7)
  "extracted_at": "2026-06-04T10:32:11Z",

  // --- blocked-only (state == "blocked"); no `value` on the cell ---
  "reason_code": "IDENTITY_UNRESOLVED", // taxonomy below (doc 10)
...
| `member` | all | the cohort-member identity (patient/encounter code); set at pending-insert, lets the agent pivot by member and bind Tier-2/3 queries without re-parsing `sources`. |
| `kind` | all | `direct` (copied) or `interpret` (inferred). |
| `state` | all | `pending` (in the ladder, not yet resolved) / `filled` / `blocked` / `not_applicable` — the four stored states (doc 5 §Cell state model). Needs-verification is **derived** (`filled` + interpret + `review_state: not_reviewed`), never stored. |
| `confidence` | all | the **word** `low` / `medium` / `high`. Drives the heat-map. No numbers. |
| `resolved_by` | all | which ladder tier produced the terminal result: `direct` / `LLM` / `agent`. |
| `hypothesis` | all | best explanation for **why the direct pass failed**, written by the first LLM to see it; `null` on a clean direct hit. |
| `attempts` | all | **list** (≥1) of attempts to fill the cell's value, in order — `{ tier, database, sql?, value?, table_column?, result?, error? }`. `database` is **always present** (which db the attempt touched, or the sentinel `"(none)"` when none did); a clean hit is one entry, no error. Tier 1's entry carries `sql` + `table_column` + the read `value`. **Write-time / evidence failures** (an off-code value, an ungroundable citation, the agent's NOT_LOCATED fallback) append an entry that carries the offending `value` + `error` and **omits `sql`** (optional — no query ran); never a synthetic SQL marker. |
...
| `sources` | all | **list**, one entry per (database, row): `{ database, query, table_column, row_id?, citations? }`. The cohort identity is the cell's `member`, **not** repeated here. The query must show the value **alongside its row identifier** (doc 6), not a bare `SELECT <value>`. `row_id` (the evidence row's own pk) is recorded for a one-to-many source or interpreted value; `citations` carries that row's **verbatim** note passage(s) — exact substrings so the highlight lands (this is where the old flat `evidence[]` now lives, linked to its row). |
| `review_state` | interpret | `not_reviewed` → `reviewed` (set automatically; doc 6 §Reviewing). |
| `corrected` | interpret | did the user edit it after reviewing? `false` confirmed · `true` corrected. |
| `prompt_version` | all | the prompt version that produced it (ties to the run record, doc 7). |
...
### Per-cell `state` (doc 5 §Cell state model; doc 10)
The four **stored** states: `pending` (inserted at run start; not yet resolved — the "open
set" the ladder works through; carries no `value`/`confidence`/`resolved_by` yet) ·
...
### Reason-code taxonomy — blocked cells (doc 10)
...
don't match are **never combined**) · `NOT_LOCATED` (the repair agent searched and could not place
...
> Lane A emits the new shape; Lane D renders it.

---

## 3. The executor spec (data, not code) — folded into `mapping.json`

**Status: FROZEN as v2 (`schema_version "2"`).** Re-frozen by A7 — the v1 `filters[]` array is
gone; filtering lives on a top-level **`cohort`** block. **Post-A4** the spec lives **inside
`mapping.json`** (the "compiled executable" half), not as a standalone `populate.json` file;
see [`storage-layout.md`](./storage-layout.md). The shape below is unchanged.

**One executor spec per audit**, embedded in `var/audits/<id>/mapping.json`.
It is **data**, never code: a single fixed, audited executor in `core/running` (Tier 1
`try_direct`) reads it, resolves the cohort, runs each region's parameterised SQL **read-only**
...
conditions are ANDed into `cohort.where` at run time (B6 resolves, B7 composes); the precompiled
...
```jsonc
{
  ...
  // Identity keys: how rows from different databases/tables are joined in Python (A3).
  // A row whose keys are missing/mismatched is NEVER combined → cell blocked (IDENTITY_UNRESOLVED).
  "identity_keys": ["patient_code"],

  // OPTIONAL — present only on multi-database mappings (A3): one bridge per foreign
  ...
  // The cohort (A7): a joinable base that SELECTs the identity keys. `from` is the anchor table
  ...
  "regions": [
    {
      ...
      // happens at run time in try_direct, not here. Each query SELECTs the identity keys.
      "queries": [
        { "database": "cord-ph",
      ...
      // source `table` (so try_direct builds the narrowed per-cell query without re-parsing
    ...
    },
    {
      "id": "NICU",
      "sheet": "NICU",
      "kind": "interpret",                   // interpret regions: fetch evidence, the agent decides

      // Interpret regions carry EVIDENCE-FETCH SQL: it returns the notes/rows for each cohort
      // member. The executor runs it (cohort-scoped) and hands the evidence to the agent, which
      // decides the value and records the verbatim passages used.
      "queries": [
        { "database": "cord-ph",
...
| `identity_bridges[]` | *(optional; multi-database only, A3)* one per foreign database: `{ database, key_column, via: { table, anchor_column, bridge_column } }` — how the anchor identity translates into that database's key. Derived from the match's `identity.keys`; single-database specs omit it. |
| `cohort` | the cohort selection (A7): `{ database, from, identity_select, where }`. `from` is a joinable base SELECTing the identity keys; `identity_select` projects the cohort key(s); `where` is the **only** per-run input — resolved inclusion conditions are ANDed in at run time and the precompiled file ships `where: []`. |
| `regions[]` | one per workbook region. |
| `regions[].kind` | `direct` (executor writes cells) or `interpret` (executor fetches evidence, agent decides). |
| `regions[].queries[]` | **read-only** SQL **per source database** — `{ database, sql, key_column? }`. SQL is scoped to the cohort via `:cohort` (its only bind) and SELECTs the identity keys. A FOREIGN-database query carries `key_column` (its own identity column; the executor binds `:cohort` with the bridged key set instead of the anchor identities). Direct: returns the values; a direct region carries ≥1 query. Interpret: returns the evidence notes/rows; an interpret region may carry **none** when its evidence lives on tables Tier 1 cannot key (higher tiers fetch it from the match's per-field sources). |
...
| `regions[].cell_map[]` | direct: `{ field, column, table, cell_template, translate? }` — result `column` → cell; `field` is the audit field id stamped on the cell, `table` lets try_direct build the narrowed per-cell query. interpret: `{ field, cell_template }` — the agent-decided field → cell. |
...
- **Cohort-scoped, never inlined.** Filtering lives on the `cohort` block: region queries bind the
  resolved cohort identity set as `:cohort`; the run ANDs the validated conditions into
...
### Run-time composition — how `:cohort` is resolved (A4 / B6 / B7)
`:cohort` is a **symbolic placeholder**, not a literal SQLite bind (the cohort is an identity
*set*). At run start the orchestrator builds the cohort selection from the `cohort` block —
`SELECT <identity_select> FROM <from> WHERE <composed where>` — and each region query's
`… IN (:cohort)` is **inlined as that subquery**. B6 resolves the user's inclusion criteria to
`where[]` entries and B7 ANDs them in; each `where[]` entry is a predicate over `cohort.from`'s
aliases (a many-valued criterion is expressed as an `EXISTS` over the anchor, per the mapping
`grain_rule`, never a count-inflating join). The run **never re-resolves** — the previewed cohort
...
Lane A (writes), Lane B (emits), and Lane D (renders) can each code against these shapes alone:

- **Lane A** can produce the `executable` block (§3) and write `cell_update.cells[].meta` (§2) — every
  field it must emit is named and typed here, with `direct`/`interpret`/`blocked` cases covered.
- **Lane B** can emit all five SSE events (§1) — payloads, ordering, and the
  `workbook_created`-early / `done`-terminal rules are fixed.
- **Lane D** can render the activity feed, the live workbook, the heat-map/markers, and the
