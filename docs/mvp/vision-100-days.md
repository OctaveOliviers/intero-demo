...
Read [README.md](./README.md) first. That spec is the **10-day MVP**: prove that a
clinician can run an audit, get a populated and traceable workbook, and trust it.

This document is the **100-day cathedral** — where the MVP goes once the core loop
works. Nothing here is in the 10-day scope. It exists so the MVP is built in a
...
The MVP proves one run. This is the standing service that run becomes.
...
or edit it (corrected), the MVP's interpretive safety gate — is a **labelled signal**.
Captured and fed back, it makes
the field mapping and the prompts measurably more accurate over time, on the Trust's
own data — something no static BI extract can do.

**Concrete shape:**
- Log each verification/correction against the cell, the source evidence, and the
  **prompt version** that produced it (the MVP already versions prompts and logs runs).
- Accumulate confirmed/corrected pairs into a per-Trust correction store.
- A refinement step feeds those pairs back into mapping generation and prompt tuning.
- The interpretive **accuracy bar** (defined in the MVP for the safety gate) becomes a
...
"self-improvement / dreaming" idea already in the backlog (TODO-0004).

**Why it's not the MVP:** it needs the core populate-and-verify loop working and
...
- A scheduler that triggers precomputed runs ahead of each deadline (this rides the
  MVP's "precompute everything that doesn't depend on the user's request" principle —
...
**Why it's not the MVP:** the MVP proves the on-demand run, which is the core motion.
...
**Concrete shape:** roles attached to accounts (doc 7's auth), and role-scoped views (a
clinician's run list vs a lead's department dashboard).

**Why it's not the MVP:** the MVP proves the single-operator run plus the
login/attribution foundation (doc 7). Role-differentiated dashboards are an overlay once
that works and once there are multiple user types on the system.

---

## V4 — True pause/resume & leave-and-return

The MVP ships **stop + re-run** (idempotent): a stopped run is re-issued and skips completed
regions. The richer experience is **true pause/resume** (freeze a live run mid-step and thaw it)
and **leave-and-return** (close the app or switch audits, come back to the run still live).

**Why it's not the MVP:** mid-session pause/resume depends on an OpenCode session capability we
haven't verified; building the UI on it would spend an innovation token on an unproven mechanic
(eng review A1, 2026-06-04). Re-run gives most of the value safely; revisit true pause/resume
once the run loop is proven.
...
surrounding feature lands; none are 10-day MVP scope.
...
v1 ships **structured-only, template-anchored** cohort filtering (see
[2-product-flows.md §Filter resolution](./2-product-flows.md#filter-resolution-and-the-cohort-count-preview)). Three capabilities are explicitly deferred here:
...
- **Free-text / interpretive cohort filtering.** Filtering the *cohort* on a concept that
  ...
  *interpret* primitive used for interpret cells, applied to an inclusion boolean), refined as
  ...
  this audit*. *(Interpret **cells** — output fields read from notes — remain in the MVP; only
  interpret **cohort filters** are deferred.)*
- **No-template ad-hoc runs / template synthesis.** Running from free text that references **no**
  pre-existing template, where Intero would **synthesise** a template (structure + fields) from
  intent (old Flow B). v1 is template-anchored: a no-match request stops and asks the user to
  upload or pick a template.
- **National-spec auto-ingestion** of canonical inclusion criteria (already a doc-9 fast-follow).
...
- **Cross-audit field-semantics reuse** (Expansion E4) lives in
  [TODOS.md](../../TODOS.md): once the schema is mapped to several templates, recurring
...
What the MVP **must** do so this vision stays reachable: version prompts, log runs
with their parameters and the clinician's verifications, and treat the interpretive
safety gate as a first-class, recorded interaction. Those three are MVP scope
