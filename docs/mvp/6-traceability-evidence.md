...
Read [2-product-flows.md](./2-product-flows.md) and [5-run-engine.md](./5-run-engine.md)
...
The promise: a clinician can click any value and see exactly how it was produced, and can
tell at a glance which values are safe copies and which the agent inferred. That is what
makes the tool trustworthy enough to put a number into a national submission.

Traceability in the MVP lives **in the app**. The richer auditable-export design in
[excel-traceability.md](./excel-traceability.md) is **deferred** (see [Export](#export));
MVP export is a plain submit-ready file.
...
| `state` | `pending` (during a run) / `filled` / `blocked` / `not_applicable` — the only **stored** states. "Needs verification" is **derived** (`filled` + interpret + `review_state: not_reviewed`), never stored (see [doc 5 §Cell state model](./5-run-engine.md#cell-state-model) and [10-status-and-blocked-items.md](./10-status-and-blocked-items.md)) |
...
| `prompt_version` | the prompt version that produced it (ties to the run record, doc 7) |
| `extracted_at` | timestamp |
| `reason_code` | **blocked only** — why the value is absent (taxonomy in [10-status-and-blocked-items.md](./10-status-and-blocked-items.md)) |
...
Clicking a cell once opens the right panel. Its content depends on `kind`.
...
**alongside the row identifier** the workbook uses (the patient/encounter code), so the
...
Three signals render on the workbook grid, together:
...
For the MVP, **export is a plain download of the populated `.xlsx`** — the template with its
...
- Members marked inactive/departed by refresh are retained in UI/run history but **excluded**
  from the exported workbook rows.
...
> **Missing values are simply empty cells.** The workbook never contains blocked-cell styling,
> in-cell annotations, or a "Blocked items" appendix — in-app or in the download
> ([10-status-and-blocked-items.md](./10-status-and-blocked-items.md)). The **download works at
