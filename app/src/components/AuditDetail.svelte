<script>
  ...
  // server accepted. Every change debounces into saveAuditCriteria; a rejected
  ...
  let saveError = "";
  let saveTimer = null;
  let criteriaAuditId = null;

  $: if (detail && detail.id !== criteriaAuditId) {
    criteriaAuditId = detail.id;
  ...
  async function persistCriteria() {
    ...
    try {
      await saveAuditCriteria(criteriaAuditId, attempt);
