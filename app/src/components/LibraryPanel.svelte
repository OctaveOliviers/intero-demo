<script>
  ...
  let datasetError = "";
  let datasetCreateText = "";
  let creatingDataset = false;
  let autoExpandDatasetId = null;
  ...
  async function handleDatasetCreate() {
    const text = datasetCreateText.trim();
    if (!text || creatingDataset) return;
    creatingDataset = true;
    datasetError = "";
    try {
      const created = await createDataset({ text });
      autoExpandDatasetId = created.id;
      datasetCreateText = "";
      await refreshDatasets();
      requestAnimationFrame(() => {
        document
          .querySelector(`[data-dataset-id="${CSS.escape(created.id)}"]`)
...
<div class="library-panel">
  <section class="content">
    {#if section === "datasets"}
      ...
      <form class="dataset-composer" on:submit|preventDefault={handleDatasetCreate}>
        <input
          class="dataset-composer-input"
          type="text"
          placeholder={$_("library.createDatasetPlaceholder")}
          bind:value={datasetCreateText}
          disabled={creatingDataset}
        />
        <button
          class="dataset-composer-button"
          type="submit"
          disabled={creatingDataset || !datasetCreateText.trim()}
        >
          {creatingDataset ? $_("library.creatingDataset") : $_("library.createDataset")}
        </button>
      </form>
      ...
      <div class="dataset-list">
        {#each datasets as d (d.id)}
          <DatasetDetail
            dataset={d}
            autoExpand={d.id === autoExpandDatasetId}
...
<style>
  ...
  .dataset-composer-input::placeholder {
    color: var(--color-text-faint);
  }

  .dataset-composer-button {
    flex-shrink: 0;
  ...
  .dataset-composer-button:hover:not(:disabled) {
    background: var(--color-accent-hover);
  }

  .dataset-composer-button:disabled {
    opacity: 0.45;
    cursor: default;
  }
