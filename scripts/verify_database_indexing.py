...
Usage:
    # Full pipeline (needs the `index_db` stage configured in models.yaml):
    python scripts/verify_database_indexing.py \
...
from data.database.scripts.build_emr_db import build_database  # noqa: E402
...
def print_model_summary(model: dict) -> None:
    ...
    for table in model.get("tables", []):
        ...
        print(
            f"  {table['name']}: {ncols} cols ({filterable} filterable), "
        ...
        for col in table["columns"]:
            ...
            print(
                f"    {col['name']:30s} [{col.get('type', '?'):7s}] {tag:10s}  {desc}"
            )
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.no_llm:
        ...
        model = asyncio.run(build_database_model(sqlite_path, args.database_id))
    ...
    seed_path = (
        REPO_ROOT / "data" / "seed" / "databases" / args.database_id / "model.json"
    )
    if seed_path.exists():
        print(
        ...
        for tname in sorted(set(seed_tables) | set(model_tables)):
            ...
            for cname in sorted(set(s_cols) | set(m_cols)):
                ...
                for key in ("filterable", "filter_type", "values", "range", "reason"):
                    ...
                    if sv != mv:
                        print(
                            f"  Δ {tname}.{cname}.{key}: seed={sv!r} vs indexed={mv!r}"
                        )
                        mismatches += 1
        if mismatches == 0:
