"""Stand up a table-agent run environment so you can drive opencode by hand.

Provisions a self-contained opencode project root at ``var/artifacts/<run_id>/``
exactly the way the production agent step does, so you can **see the agent
think** — its tool calls, its reasoning, its cell writes — without going
through the full HTTP / population stack.

The layout matches the storage contract
(:doc:`specs/mvp/contracts/storage-layout.md`).

What this script does:

1. Ensures the hospital SQLite is built (``make db`` if missing).
2. Plants the canonical files under ``var/templates/`` and ``var/databases/``
...
Run::
    ...
    python3 -m scripts.setup_table_agent_manual [--run-id <id>] [--cohort P001,P002]
"""
...
BUILT_HOSPITAL_DB = (
    REPO_ROOT / "data" / "database" / "cord-ph" / "sql" / "cord_ph.sqlite"
)

VAR_TEMPLATES_DIR = VAR_DIR / "templates"
...
def _plant_canonical_storage() -> tuple[Path, Path]:
    """Seed ``var/templates/<id>/spec.json``, ``var/databases/<id>/model.json``
    and ``var/databases/<id>/database.sqlite`` (symlinked)."""
    audit_dir = VAR_TEMPLATES_DIR / AUDIT_ID
...
def _wipe_run_dir(artifact_dir: Path) -> None:
    """Remove the per-population artifact workspace if it exists."""
...
def main() -> int:
    ...
    ap.add_argument(
        "--run-id",
        default="manual-r1",
        help="table population id (also the sub-dir under var/artifacts/)",
    )
    ap.add_argument(
        "--cohort",
        default="CPH001,CPH002",
    ...
    print(f"➤ Setting up manual table-agent run {args.run_id!r}")
    print(f"  cohort: {cohort}")

    # 1. ensure the hospital SQLite is built.
    _ensure_hospital_db()

    # 2. canonical storage seed (var/templates, var/databases).
    ...
    artifact_dir = ARTIFACTS_DIR / args.run_id
    _wipe_run_dir(artifact_dir)
    ...
    provision_worktree(run_store, artifact_dir)
    store.close()

    # 5. report.
    print()
    print(f"✓ Run ready: {artifact_dir}")
    print("  context.json:")
    for line in (artifact_dir / "context.json").read_text().splitlines():
        print(f"    {line}")
    print()
    print("  artifact workspace contents:")
    for p in sorted(artifact_dir.rglob("*")):
        rel = p.relative_to(artifact_dir)
        if any(part == "node_modules" for part in rel.parts):
            continue
        kind = (
            "symlink → " + str(p.readlink())
            if p.is_symlink()
            else ("dir" if p.is_dir() else f"file ({p.stat().st_size}B)")
        )
        print(f"    {rel}  ({kind})")
    ...
    print()
    print("━" * 70)
    print("Launch opencode from the artifact workspace:")
    print("━" * 70)
    print()
    print("  # Load repo .env into the shell, then run opencode in the workspace")
    ...
    print(
        f'  sqlite3 {STATE_DB_PATH} "SELECT ref, member, state, value '
