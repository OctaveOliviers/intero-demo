...
**Status: frozen.** This directory holds the documented **JSON Schemas** (Draft
2020-12) for the three structured models the indexing/mapping/populate pipeline writes, plus
worked examples. Everything downstream — the steps that write `spec.json` + `model.json`,
`mapping.json`, and `criteria_bindings`, plus the golden +
...
Spec source: mapping-artifact-redesign.md (PR #156),
especially §3 The three models and
§6 Defaults live in the field spec.
...
| --- | --- | --- | --- |
| [`audit-spec.schema.json`](audit-spec.schema.json) | `var/templates/<id>/spec.json` | Indexing | The **audit specification** — per-field spec + the audit's inclusion criteria. DB-agnostic. |
| [`database-model.schema.json`](database-model.schema.json) | `var/databases/<id>/model.json` | Indexing | The **database model** — schema model + the per-column filterable surface. |
| [`mapping.schema.json`](mapping.schema.json) | `var/templates/<id>/mapping.json` | Mapping | The **mapping** — audit↔database bindings: `identity`, `fields[]` (with `kind`), `criteria_bindings`, `fixed_criteria`, `database_summaries`, and the nested `executable` block. |
| [`dataset.schema.json`](dataset.schema.json) | `var/datasets/<id>/dataset.json` | Dataset lane | The **Dataset** — a saved, named filter scoping the hospital database to a slice: grounded `criteria` (each a real column + structured predicate), the `cohort` base, and the DERIVED `cohort_sql` + read-only `count`. Purely a filter; owns no data (library-and-sources.md, inclusion-criteria-setup.md). |
...
| [`cell-resolution.schema.json`](cell-resolution.schema.json) | cells (store + wire `meta`) | Run lane | The **cell object** both population steps write (prose companion: runtime-shapes.md §2). |
| [`model-config.md`](model-config.md) | `models.json` + `models.local.json` (repo root) | Platform | **Per-stage LLM model configuration** — stages, precedence, endpoint-readiness startup behaviour. |
| [`control-plane-schema-and-permissions.md`](control-plane-schema-and-permissions.md) | control-plane DB (logical) | Auth lane | Canonical IAM/catalog/runtime access model: tables, role permissions, resource grants, and DB-role boundaries. `admin` is a clinician-superset (the `clinician` set plus the admin keys; no `*`); `clinician` is all clinical staff; grants are over `dataset`/`template`/`table` with owner-driven sharing; threads/projects are not grantable; clinical access = Dataset scope ∩ hospital permissions; table-population outputs are readable by the owner or through an active grant on the wrapping table. |
| [`chat-answer.md`](chat-answer.md) | `chat-answer` skill + `cite_execute` tool + the Thread Agent runtime | Run lane (Answer) | The primary thread agent's **Answer** contract (Q43, ADR 0006): the `chat-answer` skill, streamed assistant text, the `cite_execute` evidence sink with backend-owned markers, the **chat-mode** `sql_execute` (per-message Dataset or whole-DB, no injected cohort, read-only, fail-closed — Q40), the citation/source/aggregate shape (cross-references the evidence + navigate skills), and the backend runtime seam (`core/agent/runtime.py::run_turn`). Cited by `thread.schema.json`'s `#/$defs/citation`. |
