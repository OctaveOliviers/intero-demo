...
Regression for the bug where Tier-3 agent writes (made out-of-process, straight
to state.db) never reached the frontend. There is no on-disk result.xlsx in the
spine path, so the grid is reconstructed from the cells table — making every
persisted value recoverable on reload, in a fresh browser, after navigating away.
"""
...
class WorkbookFromCellsTest(unittest.TestCase):
    def test_agent_values_recovered_from_state_db(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store = Store(db_path)
            try:
                store.create_run(Run(id="r1", audit_id="a", status="in_progress"))
                store.insert_pending_cells([
                    Cell(run_id="r1", ref="ALL!A2", field="code",
                         member="P1", kind="direct", state="pending"),
                    Cell(run_id="r1", ref="ALL!B2", field="delivery",
                         member="P1", kind="direct", state="pending"),
                ])
                src = [{"database": "db", "query": "SELECT 1", "table_column": "t.c"}]
                store.update_cell("r1", "ALL!A2", value="P1", state="filled",
                                  resolved_by="direct", confidence="high", sources=src)
                store.update_cell("r1", "ALL!B2", value="SVD", state="filled",
                                  resolved_by="agent", confidence="medium", sources=src)
            finally:
                store.close()
            ...
            with (
                patch.object(workbook_route, "AUDITS_DIR", tmp_path / "audits"),
                patch.object(workbook_route, "Store", lambda: Store(db_path)),
            ):
                resp = asyncio.run(workbook_route.get_workbook("r1"))
            ...
            self.assertEqual(resp.runStatus, "in_progress")
            self.assertIn("SVD", resp.model_dump_json())

    def test_bare_pending_seeds_excluded_from_metadata(self) -> None:
        # The full grid is seeded as `pending` before any tier runs. The live grid
        # starts from cellMetadata={} and only colors cells a tier touched, so a run
        # reopened mid-flight must NOT paint every untouched interpret cell as
        # needs-review. Snapshot metadata must match the live contract: bare seeds
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                store.insert_pending_cells([
                    Cell(run_id="r1", ref="ALL!A2", field="code",
                         member="P1", kind="interpret", state="pending"),    # bare seed
                    Cell(run_id="r1", ref="ALL!B2", field="delivery",
                         member="P1", kind="interpret", state="pending"),    # -> filled
                    Cell(run_id="r1", ref="ALL!C2", field="note",
                         member="P1", kind="interpret", state="pending"),    # attempted
                ])
                src = [{"database": "db", "query": "SELECT 1", "table_column": "t.c"}]
                store.update_cell("r1", "ALL!B2", value="SVD", state="filled",
                                  resolved_by="agent", confidence="medium", sources=src)
                store.update_cell("r1", "ALL!C2", attempts=[{"tier": "llm", "ok": False}])
            finally:
                store.close()
