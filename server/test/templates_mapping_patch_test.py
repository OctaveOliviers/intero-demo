"""PATCH /api/templates/{id}/mapping — the fixed-criteria persistence contract
...
a temp TEMPLATES_DIR, with a stub user: round-trip persist, unknown criterion_id
...
from server.models import TemplateMappingPatchRequest
...
def _user():
    # Editing inclusion criteria requires the DB-backed `template.manage`
    # permission (control-plane §4); the role is resolved from this dict, never
    # from the username.
    return {"id": "u1", "username": "clinician", "role": "clinician"}


class TemplateMappingPatchTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self.audits_dir = Path(self._tmp.name)
        self._orig_auth_db = auth_store.AUTH_DB_PATH
        auth_store.AUTH_DB_PATH = self.audits_dir / "auth.sqlite"
        auth_store.init_store()
        # #302 added owner-or-manage enforcement to patch_template_mapping: the editing
        # user (`u1`) must OWN the template (or hold a `manage` grant) in addition to
        # the `template.manage` role permission. Self-grant `manage` so these tests
        # exercise their ORIGINAL persist/validation behavior; a non-owner being
        # denied is proven in `templates_sharing_test.py`.
        grant_store.self_grant_manage(
            resource_type="template", resource_id="cord-ph", user_id="u1"
        )
        ...
        assert self.valid_entry is not None, (
            "seed mapping must carry a fixed_criteria example"
        )
    ...
    async def _call(self, fixed_criteria, *, user=..., read_only=False):
        body = TemplateMappingPatchRequest(fixed_criteria=fixed_criteria)
        resolved_user = _user() if user is ... else user
        with (
            patch.object(templates_routes, "TEMPLATES_DIR", self.audits_dir),
            patch.object(
                templates_routes, "get_audit_by_id", return_value=_audit_row(read_only)
            ),
            patch.object(templates_routes, "current_user", return_value=resolved_user),
        ):
            return await templates_routes.patch_template_mapping(
                "cord-ph", body, _FakeRequest()
            )
    ...
    async def test_round_trip_persists_and_rereads(self):
        ...
        self.assertEqual(
            on_disk["criteria_bindings"], self.mapping["criteria_bindings"]
        )
