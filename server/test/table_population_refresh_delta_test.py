...
class RefreshDeltaTest(unittest.TestCase):
    ...
    def _create_execution(self, execution_id: str) -> None:
        self.store.create_execution(
            RunExecution(id=execution_id, run_id="r1", status="running")
        )
    ...
    def test_refresh_delta_new_and_departed_members(self):
        ...
        stats = table_populations_route._prepare_refresh_delta(
            self.store,
            table_population_id="r1",
            execution_id="exec-1",
    ...
    def test_refresh_delta_keeps_filled_and_needs_verification_unchanged(self):
        ...
        table_populations_route._prepare_refresh_delta(
            self.store,
            table_population_id="r1",
            execution_id="exec-1",
            executable=EXECUTABLE,
        ...
        self.assertEqual(
            (after_filled.state, after_filled.value, after_filled.resolved_by),
        ...
        self.assertEqual(
            (
                after_needs.state,
                after_needs.value,
                after_needs.review_state,
                after_needs.corrected,
            ),
            (
                before_needs.state,
                before_needs.value,
                before_needs.review_state,
                before_needs.corrected,
            ),
        )
    ...
    def test_refresh_summary_counters(self):
        ...
        summary = table_populations_route._build_refresh_summary(
            self.store,
            table_population_id="r1",
            execution_id="exec-1",
            refresh_stats=stats,
