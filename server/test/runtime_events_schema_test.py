...
machine contract (docs/mvp/contracts/runtime-events.schema.json) — the same
schema the front-end mock timelines are tested against, so the two sides
cannot drift apart silently.

Canonical builder outputs are validated directly; payload shapes the spine
...
server/routes/runs.py and core/running/orchestrator.py.
"""
...
from core.running.events import build_activity_event, build_review_summary_event

_SCHEMA_PATH = (
    Path(__file__).resolve().parents[2]
    / "docs" / "mvp" / "contracts" / "runtime-events.schema.json"
)
...
class RuntimeEventsSchemaTest(unittest.TestCase):
    def test_activity_builder_outputs_validate(self):
        ...
        _assert_valid(build_activity_event(
            "Reading the notes.", detail="Per-cell evidence.",
            name="sql_execute", status="running",
        ))
    ...
    def test_review_summary_builder_output_validates(self):
        _assert_valid(build_review_summary_event([]))

    def test_workbook_created_shape_validates(self):
        # Mirrors the emit in server/routes/runs.py::_run_spine.
        _assert_valid({
    ...
    def test_cell_update_shape_validates(self):
        # Mirrors core/running/orchestrator.py RunStore.update's payload.
        _assert_valid({
            "type": "cell_update",
            "sheet": "ALL",
            "cells": [{"ref": "A2", "value": "P-0001", "meta": {
                "field": "all/patient_code", "member": "P-0001",
                "kind": "direct", "state": "filled", "confidence": "high",
            }}],
        })

    def test_refresh_summary_and_done_shapes_validate(self):
        # Mirrors server/routes/runs.py::_build_refresh_summary.
        delta = {
            "new_members_count": 2,
    ...
    def test_error_shape_validates(self):
        _assert_valid({"type": "error", "message": "Run stopped by user."})
        _assert_valid({
            "type": "error", "message": "Database connection lost.",
            "scope": "NICU!region:outcomes", "executionId": "exec-2",
        })
