"""Tests for DELETE /api/table-populations/{id}: stop-then-delete + full teardown.

Deleting a run must leave NO trace: the runs row and all its cascaded children
(cells, events, …) in state.db, the var/artifacts/<id> directory, and the auth.sqlite
attribution row. It must also stop the live execution (opencode session + session
...
from core import table_population as table_population_module
...
class DeleteTablePopulationTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._orig_relay = table_population_module.get_session_relay()
        table_population_module.set_session_relay(None)
    ...
    def tearDown(self) -> None:
        ...
        table_population_module.set_session_relay(self._orig_relay)
    ...
    def _seed_run(self, *, run_id: str, owner_user_id: str | None) -> None:
        ...
        try:
            ...
            store.close()
        # The on-disk run directory the agent/session would have created.
        run_dir = self.runs_dir / run_id
        (run_dir / "template").mkdir(parents=True, exist_ok=True)
    ...
    def test_owner_delete_removes_every_trace(self):
        ...
        resp = asyncio.run(
            route.delete_table_population("r1", self._request_for(self.owner_user))
        )
    ...
    def test_non_owner_is_denied_and_nothing_is_deleted(self):
        ...
        with self.assertRaises(HTTPException) as ctx:
            asyncio.run(
                route.delete_table_population("r2", self._request_for(self.other_user))
            )
    ...
    def test_unauthenticated_returns_401(self):
        ...
        with self.assertRaises(HTTPException) as ctx:
            asyncio.run(route.delete_table_population("r3", self._request_for(None)))
        self.assertEqual(ctx.exception.status_code, 401)

    def test_stop_then_delete_aborts_session_and_cancels_session_task(self):
        self._seed_run(run_id="r4", owner_user_id=self.owner_user["id"])
        stopped: list[str] = []

        class FakeRelay:
            async def abort_session(self, run_id: str) -> bool:
                stopped.append(run_id)
                return True

        async def scenario():
            table_population_module.set_session_relay(FakeRelay())
            task = asyncio.ensure_future(asyncio.sleep(10))
            route._TABLE_POPULATION_SESSION_TASKS["r4"] = task
            resp = await route.delete_table_population(
                "r4", self._request_for(self.owner_user)
            )
            return resp, task
        ...
        self.assertTrue(task.cancelled())  # session task cancelled
        self.assertNotIn(
            "r4", route._TABLE_POPULATION_SESSION_TASKS
        )  # task handle forgotten
